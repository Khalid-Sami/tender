...
//            ])->count();
    ...
    public function getItems(){
        $data = ItemsView::with(['category'])->get();
        return Datatables::of($data)->make(true);
    }
...
//        return view('admin.editItem',$data)->with(['categoriesIDS' => $categoriesIDS]);
    ...
    public function getAllItems($id){
        ...
        if($id == 0)
            ...
            $data = ItemsView::where('fk_i_cat_id',$id)->with(['category'])->get();
...
//                ]);
        ...
        public function getItemsUnderCategory($id){
            $items = ItemsView::where('i_parent_cat_id',$id)->with(['unit'])->get();
        ...
        public function getOfferDetails($id){
            ...
            return view('admin.offerDetails', compact('offer'));
        }
...
//            PDF::Write(0, 'Hello World');
        ...
        public function tryPusher(){
            $options = array(
                'encrypted' => true
            );
            $data['message'] = 'try pusher';
            $pusher = new \Pusher(
                env("PUSHER_KEY"),
                env("PUSHER_SECRET"),
                env("PUSHER_APP_ID"),
            ...
            $pusher->trigger('auction', 'adoptAuction', $data);
        }
