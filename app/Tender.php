...
//        Carbon::setLocale($lang);
    ...
    public function tenderItems(){
        return $this->hasMany('App\TenderItems', 'fk_i_tender_id', 'pk_i_id');
    }

    public function attachments(){
        return $this->hasMany('App\Attachments', 'fk_i_refe_id','pk_i_id')->where('i_attach_type',1);
    }

    public function tenderProposals(){
        return $this->hasMany('App\TenderProposal', 'fk_i_tender_id','pk_i_id')->with(['company','proposalItems']);
    }

    public function tenderProposal(){
        return $this->hasMany('App\TenderProposal', 'fk_i_tender_id','pk_i_id');
    }

    public function accetpoffer(){
        return $this->belongsTo('App\TenderProposal', 'i_accept_offer', 'pk_i_id')->with(['company']);
    }
