        ...
        <div class="portlet box blue">
            ...
            <div class="portlet-body" style="display: block;">
                <div class="row">
                    ...
                    $status = $help->getQuotationStatus($requests->pk_i_id);
                    ?>
                    @if($status != 65)
                        @if($requests->i_status != 59)
                            <div class="col-sm-12">
                    ...
                    @if($status == 65)
                        @if($requests->i_status != 59)
                            @if($isEvaluated != 0)
                                    <div class="alert alert-success alert-dismissible text-center">
                                        <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                        @lang('lang.request_done')
                                    </div>
                            @else
                                <div class="col-sm-12">
                                    <a href="{{route('rating.show',[$requests->pk_i_id])}}"
                                       class="btn btn-success pull-right ">@lang('lang.mark_as_completed')</a>
                                </div>
                    ...
                    <div class="col-sm-12">
                        <h3>@lang('lang.status'): {{$requests->status->s_name}}</h3>
        ...
        @foreach($requestQuotation as $quotation)
            <div class="mt-timeline-content col-sm-10">
                <div class="mt-content-container bg-white  border-grey-steel"
                     style="{{$quotation->i_status == 65?'border: 2px solid #000000 !important':'border: 2px solid #d3d7e9'}};padding: 40px;">
                    <div class="mt-author">
                        <div class="mt-avatar">
                            @if($quotation->i_status == 65)
                                <button class="btn btn-circle green-jungle  pull-right"
                                        type="button"> @lang('lang.chosen')
                                </button>
                            @endif
                    ...
                    <div class="mt-content border-grey-steel">
                        ...
                        <div class="pull-right">
                            <a href="{{ route('user.quotation.show',$quotation->pk_i_id) }}"
                               class="btn btn-circle blue-steel dropdown-toggle moreDetails" type="button"
                               data-target="#myModal" data-toggle="modal"> @lang('lang.more')
                            </a>
                            <a href="{{ route('message.show',$quotation->serviceProvider->pk_i_id) }}"
                               class="btn btn-circle blue-steel dropdown-toggle moreDetails"
                               type="button"> @lang('lang.chat')
                            </a>
                        </div>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script>
        $(function () {
        var status = parseInt("{{$requests->i_status}}");
        var isEvaluated = parseInt("{{$isEvaluated}}");
        if (status == 58 && isEvaluated == 0) {
        window.location.href = "{{ route('rating.show',$requests->pk_i_id) }}";
        }
        });
    </script>
    <script>
