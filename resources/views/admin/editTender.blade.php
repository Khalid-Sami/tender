...
@section('content')
    ...
    <form method="POST" id="newTenderForm" action="{{ action('AdminController@setEditTender',$tender->pk_i_id) }}" enctype="multipart/form-data">
        ...
        <div class="row portlet-body form-group">
            ...
            <div class="col-sm-12">
                <div class="form-group col-md-12">
                    ...
                    <input id="tenderTitle" name="tenderTitle" type="text" class="form-control" value="{{ $tender->s_title }}">
            ...
            <div class="col-sm-12">
                <div class="form-group col-sm-6">
                    <label for="startDatepicker" class="control-label">{{ trans('lang.tenderStartDate') }}</label>
                    <div class="input-group date form_meridian_datetime form_datetime bs-datetime" id="startDateTender" data-date="2017-06-11T15:25:00Z">
                        <input id="startDate" name="startDate" type="text" size="16" class="form-control" value="{{ $tender->dt_open_date }}">
                ...
                <div class="form-group col-sm-6">
                    <label for="endDatepicker" class="control-label">{{ trans('lang.tenderEndDate') }}</label>
                    <div class="input-group date form_meridian_datetime form_datetime bs-datetime" data-date="2017-06-11T15:25:00Z">
                        <input id="endDate" name="endDate" type="text" size="16" class="form-control" value="{{ $tender->dt_close_date }}">
                        <span class="input-group-btn">
                        <button class="btn default date-set" type="button">
                            <i class="fa fa-calendar"></i>
                        </button>
            ...
            <div class="col-sm-12">
                <div class="form-group col-sm-12">
                    ...
                    <textarea id="editor1" class="wysihtml5 form-control" rows="6" name="editor1" data-error-container="#editor1_error">{{ $tender->s_description }}</textarea>
            ...
            <div class="col-sm-12">
                <div class="form-group col-sm-12">
                    ...
                    {{--<textarea id="tenderDetails" name="markdown" data-provide="markdown" rows="5" data-error-container="#editor_error"></textarea>--}}
                    <textarea id="tenderTerms" class="wysihtml5 form-control" rows="6" name="tenderTerms" data-error-container="#tenderTerms_error">{{ $tender->s_terms }}</textarea>
                    ...
                    <div class="storedListFiles">
                        @foreach($tender->attachments as $attachment)
                            <span class="storedFile">
                                <a id="{{$attachment->pk_i_id}}" class="removeFile"><span class="glyphicon glyphicon-remove-sign"></span><a class="file" href="{{ asset('/files/tender/'.$attachment->s_url.'') }}">{{ $attachment->s_name }}</a></a>&nbsp;
                            </span>
                ...
                <div class="form-group col-sm-12">
                    ...
                    <select id="currency" name="currency" class="form-control select">
                        ...
                        @foreach($currencies as $currency)
                            @if($currency->pk_i_id == $tender->fk_i_currency_id)
            ...
            <div class="col-sm-12" style="padding-top: 50px">
                <div class="col-sm-12">
                    <table id="itemsTable" class="col-sm-12" border="1" cellspacing="0">
                        ...
                        @foreach($tender->tenderItems as $key=>$tenderItem)
                            <tr id="{{$tenderItem->fk_i_item_id}}" class="{{$tenderItem->pk_i_id}}">
                                <td><a class="removeItem"><span class="glyphicon glyphicon-remove-circle"></span></a></td>
                                <td>{{$key+1}}</td>
                                <td>{{$tenderItem->item->s_name}}</td>
                                <td>{{$tenderItem->item->unit->s_name}}</td>
                                <td><input name="quantityItem[{{ $key+1 }}]" type="text" class="form-control quantityItem" value="{{$tenderItem->i_quantity}}"></td>
                                <td><input name="itemNotes[{{ $key+1 }}]" type="text" class="form-control itemNotes" value="">
                                    <input type="hidden" name="ids[{{ $key+1 }}]" value="{{$tenderItem->pk_i_id}}">
                                    <input type="hidden" name="itemsIds[{{ $key+1 }}]" value="{{$tenderItem->fk_i_item_id}}">
                                    </td>
                                </tr>
                        @endforeach
                        </tbody>
...
@section('scripts')
    <script>
        $('#document').ready(function () {
            ...
            $('#selectItems').change(function(){
                ...
                if($(this).val() != "" && !status){
                    ...
                    $('#itemsTable tbody').append([
                        ...
                        '<input type="hidden" name="itemsIds['+(parseInt(index)+1)+']" value="'+id+'">' +
                        '</td>',
                        '</tr>'
                    ].join(''));
                }
            ...
            $('body').on('click','.removeItem',function () {
                ...
                {{--else {--}}
                        {{--$.ajax({--}}
                            {{--method:'GET',--}}
                            {{--dataType: 'json',--}}
                            {{--url: '{{url('/')}}/admin/removeItem',--}}
                            {{--data:{--}}
                                {{--"_token": "{{csrf_token()}}",--}}
                                {{--"itemID": $(this).closest('tr').attr('class')--}}
                            {{--},--}}
            ...
            {{--@foreach($tender->tenderItems as $key=>$tenderItem)--}}
                    {{--$('#selectItems option').filter('[value="{{$tenderItem->fk_i_item_id}}"]').prop('selected', true);--}}
{{--                 $("#selectItems option[value='" + {{$tenderItem->fk_i_item_id}} + "']").prop("selected", true);--}}
                    {{--$('#itemsTable tbody').append([--}}
                        {{--'<tr id="{{$tenderItem->fk_i_item_id}}" class="{{$tenderItem->pk_i_id}}">',--}}
                        {{--'<td><a class="removeItem"><span class="glyphicon glyphicon-remove-circle"></span></a></td>',--}}
                        {{--'<td>{{$key+1}}</td>',--}}
            ...
            {{--$.each('{{$tender->tenderItems->fk_i_item_id}}', function(i,e){--}}
                {{--$("#selectItems option[value='" + e + "']").prop("selected", true);--}}
            {{--});--}}
...
//        e.preventDefault();
            ...
            var tenderProposal = '{{$tender->pk_i_id}}';
            swal({
