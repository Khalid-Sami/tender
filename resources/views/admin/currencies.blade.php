...
@section('content')
    ...
    <!-- /.modal -->
    <div id="showCurrency" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            {!! Form::open(['id'=>'editCurrency','method'=>'PATCH','action'=>'AdminController@editConstant']) !!}
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">@lang('lang.editCurrency')</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        {!! Form::hidden('pk_i_id',null,['id'=>'c_pk_i_id']) !!}
                        {!! Form::hidden('key','CURRENCY') !!}
{{--                        {!! Form::hidden('parentID',null,['id'=>'parentID']) !!}--}}
                        <div class="form-group col-sm-6">
                            {!! Form::label('name_ar',trans('lang.currencyArabic')) !!}
                            {!! Form::text('name_ar',null,['class'=>'form-control','id'=>'currencyArabic']) !!}
                            <div id="name_ar_validate" class="font-red"></div>
                        </div>
                        <div class="form-group col-sm-6">
                            {!! Form::label('name_en',trans('lang.currencyEnglish')) !!}
                            {!! Form::text('name_en',null,['class'=>'form-control','id'=>'currencyEnglish']) !!}
                    ...
                    <div class="row">
                        <div class="form-group col-sm-12">
                            ...
                            {!! Form::select('c_status',isset($status)?$status:[],null,['class'=>'form-control select','id'=>'currency_status']) !!}
    ...
    <div id="addCurrencyModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            {!! Form::open(['id'=>'addCurrencyForm','method'=>'POST','action'=>'AdminController@newConstant']) !!}
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">@lang('lang.addNewCurrency')</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
{{--                        {!! Form::hidden('fk_i_parent_id',8) !!}--}}
                    ...
                    <div class="row">
                        <div class="form-group col-sm-12">
                            ...
                            {!! Form::select('c_status',isset($status)?$status:[],null,['class'=>'form-control select', 'id' => 'currencyStatus']) !!}
...
@section('scripts')
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#myTable').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{!! route('getConstant','CURRENCY') !!}"
                },
                "columns": [
                    {
    ...
    <script>
        $('body').on('click', '.edit_currency', function () {
            ...
            $('#currencyArabic').val(s_name_ar);
            $('#currencyEnglish').val(s_name_en);
            $('#c_pk_i_id').val(pk_i_id);
            $('#currency_status').val(status);
            $('#currency_status').select2();
            $('#showCurrency').modal('show');

        });
    </script>
    <script>
        $('body').on('click', '#addCurrency', function () {
            $('#addCurrencyModal').modal('show');
//            $('#categoryStatus').val(1);
            $('#currencyStatus').select2();
            $('#addCurrencyForm').validate({
                ...
                messages: {
                    @if(app()->getLocale() =='ar')
                    name_ar: "العملة حقل مطلوب",
                    name_en: "العملة حقل مطلوب (English)",
                    c_status: " الحالة حقل مطلوب",
                    @else
                    name_ar: "Currency (Arabic) field is required",
                    name_en: "Currency field is required",
