...
@endsection
@section('title')
    {!!  trans('lang.constant_management')." /<small>$name1</small>"." /<small>$name2</small>" !!}
@endsection
@section('msg')
    @if(session('msg'))
...
@section('content')
    <div class="col-sm-12">
        <button class="btn green" id="addDistrict"> <span class="fa fa-plus"></span> @lang('lang.district')</button>
    ...
    <div id="editDistrictModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            {!! Form::open(['id'=>'editDistrictForm','method'=>'PATCH','action'=>'AdminController@updateConstant']) !!}
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">@lang('lang.edit_district')</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        {!! Form::hidden('pk_i_id',null,['id'=>'c_pk_i_id']) !!}
                        <div class="form-group col-sm-6">
                            {!! Form::label('name_ar',trans('lang.district_ar')) !!}
                            {!! Form::text('name_ar',null,['class'=>'form-control','id'=>'district_ar']) !!}
                            <div id="name_ar_validate" class="font-red"></div>
                        </div>

                        <div class="form-group col-sm-6">
                            {!! Form::label('name_en',trans('lang.district_en')) !!}
                            {!! Form::text('name_en',null,['class'=>'form-control','id'=>'district_en']) !!}
                    ...
                    <div class="row">
                        <div class="form-group col-sm-6">
                            ...
                            {!! Form::select('c_status',isset($status)?$status:[],null,['class'=>'form-control select','id'=>'district_status']) !!}
    ...
    <div id="addDistrictModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            {!! Form::open(['id'=>'addDistrictForm','method'=>'POST','action'=>'AdminController@storeConstant']) !!}
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">@lang('lang.add_district')</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        {!! Form::hidden('fk_i_parent_id',isset($city_id)?$city_id:'') !!}
                        {!! Form::hidden('key','DISTRICTS') !!}
                        <div class="form-group col-sm-6">
                            {!! Form::label('name_ar',trans('lang.district_ar')) !!}
                    ...
                    <div class="row">
                        <div class="form-group col-sm-6">
                            ...
                            {!! Form::select('c_status',isset($status)?$status:[],null,['class'=>'form-control select']) !!}
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            $('#myTable').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{!! route('constant.get',['ConstantView',$city_id]) !!}"
                },
    ...
    <script>
        $('body').on('click', '.edit_city', function () {
            ...
            $('#district_ar').val(s_name_ar);
            $('#district_en').val(s_name_en);
            $('#c_pk_i_id').val(pk_i_id);
            $('#district_status').val(status);
            $('#district_status').select2();
    ...
    <script>
        $('#editDistrictForm').validate({
            ...
            messages: {
                @if(app()->getLocale() =='ar')
                name_ar: "الحي حقل مطلوب",
                name_en: "الحي حقل مطلوب (English)",
                c_status: " الحالة حقل مطلوب",
                @else
                name_ar: "district (Arabic) field is required",
                name_en: "district field is required",
                c_status: "status field is required"
                @endif
            }, submitHandler: function (form) {
                form.submit();
