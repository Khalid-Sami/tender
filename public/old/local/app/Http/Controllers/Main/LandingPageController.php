...
//        $this->middleware(['locale']);
    ...
    private function getServiceProviderLocations()
    {
        ...
        return ServiceProviderView::where('dt_deleted_date', null)->get(['s_address']);
    ...
    {
        $result = ServiceProviderView::where(['dt_deleted_date' => null, 's_address_en' => $address])->get()
...
//        var_dump($services);
    ...
    public function changeStatus(Request $request, $requestId, $serviceProviderId)
    {
        ...
        if ($method == 'accept') {
            ...
            RequestQuotation::where(['fk_i_request_id' => $requestId, 'fk_i_service_provider_id' => $serviceProviderId])->update(['i_status' => 67, 'dt_modified_date' => Carbon::now()]);
    ...
    public function storeQuotation(Request $request, $requestId)
    {
        ...
        if (empty($requestQuotation)) {
            ...
            $quotation = $this->updateRequestQuotation($request, $requestId, $serviceProviderId, $quotationId);
            RequestProposedSP::where(['fk_i_request_id' => $requestId, 'fk_i_service_provider_id' => $serviceProviderId->fk_i_service_provider_id])->update(['i_replied' => 1, 'dt_replied' => Carbon::now()]);
            if ($request->hasFile('file')) {
                $files = $request->file('file');
                foreach ($files as $file) {
