...
//       $userPermission->createUserPermission($user->pk_i_id, $patientPermission);
    ...
    public function storeServiceProvider(Request $request)
    {
        $this->validate($request,[
            ...
            'service_provider_telephone_number' => 'required|unique:t_service_provider,s_telephone_number',
            'service_provider_email' => 'required|email|unique:t_service_provider,s_email',
            'service_provider_mobile_number' => 'required|unique:t_service_provider,s_mobile_number'
        ]);
...
//        $userPermission->createUserPermission($user->pk_i_id, $clinicPermission);
        ...
        if($admin_user->s_default_language == 'en'){
            $english_message = "New Service Provider Account has been registered";
        ...
        if($admin_user->s_default_language == 'ar'){
            $arabic_message = "تم تسجيل حساب مزود خدمة جديد جديد";
